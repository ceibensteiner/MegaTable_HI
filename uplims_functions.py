...
def get_new_base(base_dir,base,galaxyID,base_dir_out):
    ...
    #get quantities
    hi = gal["Sigma_atom_MeerKAT"]
    hi_unc = gal["e_Sigma_atom_MeerKAT"]
    co = gal["Sigma_mol"]
    co_unc = gal["e_Sigma_mol"]
    rgal_kpc = gal["r_gal"]
    ...
    lines_f  = np.array(['Sigma_atom','Sigma_mol']) #eigentlich sigma aber sonst müsste ich unten alles ändern
    ...
    fname_dict = base_dir_out+galaxyID+'_base+MeerKAT+P-DE_hexagon_1p5kpc_uplims'+'.ecsv'
...
def get_new_base_mhongoose(base_dir,base,galaxyID,base_dir_out):
    ...
    #get quantities
    hi = gal["Sigma_atom_MHONGOOSE"]
    hi_unc = gal["e_Sigma_atom_MHONGOOSE"]
    co = gal["Sigma_mol"]
    co_unc = gal["e_Sigma_mol"]
    rgal_kpc = gal["r_gal"]
